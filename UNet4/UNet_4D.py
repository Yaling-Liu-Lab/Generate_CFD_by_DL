...
#         BC_lowerWall_R = ini[:, 0:3, -1:, end_outlet:]
        ...
        geo = geometry[0,0,:,:].detach().cpu().numpy() # Detach the geometry
...
#         plt.imshow(X_loss.detach().cpu().numpy()[0,0,:,:])
#         print(X_loss.abs().mean(), Y_loss.abs().mean(), P_loss.abs().mean())
...
#         PMAX, PMIN = 8.5, -2.5
        ...
        P_results = P_results * geometry_bd_excluded
